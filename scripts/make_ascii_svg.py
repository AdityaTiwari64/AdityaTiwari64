...
Usage:
    ...
    Defaults: source-prepped.png -> aditya-ascii.svg
    Run prep_photo.py first to produce source-prepped.png from your photo.
"""
...
# Light global tuning (heavy lifting was already done by prep_photo.py)
CONTRAST    = 1.05
BRIGHTNESS  = 1.0
GAMMA       = 1.18         # >1 brightens mids -> face lands in sparser chars
SHARPEN     = False
WHITE_FLOOR = 0.80         # luminance above this is forced to space (blank bg)
...
def sample_image():
    """Load image and sample it into a COLS x ROWS character grid."""
...
def build_svg(rows_txt):
    ...
    parts.append(f'<style>text{{font-family:{mono};font-size:{CELL_H - 2}px;line-height:1;}}</style>')
    ...
    for ri, row in enumerate(rows_txt):
        ...
        if STATIC:
            parts.append(f'<text x="{art_x}" y="{y}" fill="{INK}">{text}</text>')
        else:
            clip_id = f"cr{ri}"
            parts.append(f'<text x="{art_x}" y="{y}" fill="{INK}" clip-path="url(#{clip_id})">{text}</text>')
...
def main():
    if not os.path.exists(SRC):
        ...
        print("Run prep_photo.py first:  python scripts/prep_photo.py <your-photo.jpg>")
        sys.exit(1)
